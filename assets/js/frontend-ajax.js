...
function Loading(){
    $('.productSingle_loading').html('<div class="productSingle_loading-inner">\n' +
    ...
    //change gallery in Single product ------------------

    $('.select-options li').click(function () {
        var pro_id = $('#size').attr('pro_id');
        var lang = $('#size').attr('lang');
        ...
        $.ajax({
            ...
            data: {
                action: 'gallery_models',
                index: index,
                pro_id: pro_id,
            },
            success: function (response) {
                if (parseInt(response.count) >= 0) {
                    $('.large-img').html(response.content);
                    $('.thumbnail-img').html(response.content_thumb);
                    $('.productSingle_loading').html('');
                    ...
                    var pitpHeight = $('.productSingle-image .carousel-nav .swiper-slide').outerWidth();
                    $('.productSingle-image .carousel-nav .swiper-slide').css('height', pitpHeight);
                    var pdding = (lang=='fa')? 'padding-left': 'padding-right';
    ...
    $('.product-filter').click(function () {
        var selected = [];
        Loading();
        jQuery('.productFilter-panel-item-list input:checked').each(function () {
            selected.push($(this).attr('data-id'));
        });
       var lang = $('.productFilter-panel').attr('lang');
        $.ajax({
            url: frontend_ajax_object.ajaxurl,
            type: 'post',
            dataType: 'json', //Type of Response
            ...
            success: function (response) {
                if (parseInt(response.count) >= 0) {
                    $('.productsBoxes').html(response.content);
                    $('.page-numbers').html(response.number);
                    $('.total-page').html(response.count);
    ...
    $('.hvr-pulse').click(function (e) {
        e.preventDefault();
        var offset = $(this).attr('current');
        var selected = [];
        Loading();
        jQuery('.productFilter-panel-item-list input:checked').each(function () {
            selected.push($(this).attr('data-id'));
        ...
        $.ajax({
            ...
            success: function (response) {
                if (parseInt(response.count) >= 0) {
                    $('.productsBoxes').append(response.content);
                    $('.page-numbers').html(response.number);
                    $('.total-page').html(response.count);
                    var piHeight = $('.productsBox-image').outerWidth() * 1.35;
                    $('.productsBox-image').css('height', piHeight);
    ...
    $('.blog-category').click(function (e) {
        ...
        e.preventDefault();
        $('.blogContent').prepend('<div class="productSingle_loading"></div>');
        Loading();
        var selected = [];
